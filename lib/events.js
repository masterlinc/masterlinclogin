...
// 2026-08-08 自检漏斗：selfcheck_start / selfcheck_result / pay_click；首页产品卡 product_click
// 2026-08-09 免费漏斗：consult_click（免费咨询/先聊 5 分钟入口点击，extra: where/source）
// 2026-08-09 v1.31.0 测量补强：transition_enter（驾驶舱进入，extra: where/source）+ home_cta_click（首页主 CTA，extra: where/target）
const EVENT_ALLOW = new Set([
  'page_view', 'skill_download', 'lead_submit', 'skill_pack_submit', 'feedback_submit',
  'selfcheck_start', 'selfcheck_result', 'pay_click', 'product_click', 'consult_click',
  'transition_enter', 'home_cta_click'
]);

// extra 字段白名单（对齐行为日志方案 §3.3 + Skill 下载口径追加）
const EXTRA_ALLOW = new Set([
  // 通用
  'path', 'title', 'from', 'to', 'where', 'which', 'step', 'sel', 'source', 'target',
  // 自检 / 线索
